    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            ...
            if(b[i][j] == opposite_player){
                opposite_disc++;
                opposite_weight += weight_point[i][j];
            }
        }
    }
    int space = 64 - player_disc - opposite_disc;
    if(space > 0) return player_weight - opposite_weight;
    else return player_disc - opposite_disc;
}
