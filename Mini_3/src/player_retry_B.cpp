...
int count_value(Board b){
    int count_disc[3] = {};
    int player_weight = 0;
    int opposite_weight = 0;
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            if(b[i][j] == player){
                count_disc[player]++;
                player_weight += weight_point[i][j];
            }
            if(b[i][j] == opposite_player){
                count_disc[opposite_player]++;
                if(i == 0 &&(j == 0 || j == 7)) opposite_weight += 1225;
                if(i == 7 &&(j == 0 || j == 7)) opposite_weight += 1225;
            }
        }
    }
    return count_disc[player] - count_disc[opposite_player] + player_weight - opposite_weight;
}
